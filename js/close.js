...
function sanitizeSettings(){
  ...
  if(state.settings.cashiers===undefined) state.settings.cashiers = [];
}
...
function renderCashierReportFilter(){
  const sel = $("repCashierFilter");
  if(!sel) return;
  ...
  sel.value = cashiers.includes(current) ? current : "";
}

function normalizeCashierList(raw){
  let list = [];
  if(Array.isArray(raw)) list = raw.slice();
  else if(typeof raw === "string") list = raw.split(/[\n,]+/g);
  const seen = new Set();
...
function getCashiers(){
  return normalizeCashierList(state.settings?.cashiers || []);
}

let __cashierResolver = null;

function closeCashierModal(name){
  const ov = $("cashierModalOverlay");
  if(ov){
    ov.classList.remove("show");
    ov.setAttribute("aria-hidden", "true");
    ov.style.display = "none";
  }
  if(__cashierResolver){
    __cashierResolver(name || "");
    __cashierResolver = null;
  }
}

function chooseCashierForCheckout(){
  const cashiers = getCashiers();
  if(!cashiers.length){
    alert("Primero agrega personas que cobran en Ajustes.");
    setView("settings");
    setTimeout(()=>{ const el = $("sCashiers"); if(el) el.focus(); }, 80);
    return Promise.resolve("");
  }

  return new Promise((resolve)=>{
    __cashierResolver = resolve;
    const box = $("cashierButtons");
    const ov = $("cashierModalOverlay");
    if(!box || !ov){
      resolve(cashiers[0] || "");
      return;
    }
    box.innerHTML = cashiers.map(name =>
      `<button class="btn primary cashier-choice" type="button" data-cashier="${escapeHtml(name)}">${escapeHtml(name)}</button>`
    ).join("");
    box.querySelectorAll("button[data-cashier]").forEach(btn=>{
      btn.addEventListener("click", ()=>closeCashierModal(btn.dataset.cashier || ""), {once:true});
    });
...
async function loadSettings(){
  ...
  if($("sCashiers")) $("sCashiers").value = getCashiers().join("\n");
...
async function saveSettings(){
  ...
  const cashiersRaw = ($("sCashiers")?.value || "").trim();
  ...
  state.settings.cashiers = normalizeCashierList(cashiersRaw);
