...
function isAdminSessionOpen(){
  try{ return sessionStorage.getItem(EDIT_OK_KEY) === "1"; }catch(e){ return false; }
}

function syncAdminSessionUI(){
  const open = isAdminSessionOpen();

  const badge = document.getElementById("adminOpenBadge");
  if(badge){
...
function openAdminSession(){
  try{ sessionStorage.setItem(EDIT_OK_KEY, "1"); }catch(e){}
  syncAdminSessionUI();
}

function requireCode(action){
  // ya autorizado en esta pestaña
  if(isAdminSessionOpen()) return true;
...
document.addEventListener("DOMContentLoaded", syncAdminSessionUI);
window.addEventListener("pageshow", syncAdminSessionUI);
document.addEventListener("visibilitychange", ()=>{
  if(!document.hidden) syncAdminSessionUI();
});
