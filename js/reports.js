...
function reportMethodKey(method){
  const key = String(method || "").trim().toUpperCase();
  if(key === "TRANSFER" || key === "TRANSFER.") return "TRANSFERENCIA";
  return key;
}

function addReportPayment(pay, method, amount){
  const key = reportMethodKey(method);
  const value = Number(amount || 0);
  if(!key || !isFinite(value)) return;
  pay[key] = Number((Number(pay[key] || 0) + value).toFixed(2));
}

function calcReportPaymentTotals(list){
  const pay = { EFECTIVO:0, TARJETA:0, TRANSFERENCIA:0, MIXTO:0, OTROS:0 };
  for(const s of (list || [])){
    if(s?.payMethod === "MIXTO" && s?.paySplit && typeof s.paySplit === "object"){
      for(const key of Object.keys(s.paySplit)){
        addReportPayment(pay, key, s.paySplit[key]);
      }
      continue;
    }
    if(s?.payMethod === "MIXTO"){
      addReportPayment(pay, "MIXTO", s.total);
      continue;
    }
    const key = reportMethodKey(s?.payMethod);
    if(key === "EFECTIVO" || key === "TARJETA" || key === "TRANSFERENCIA") addReportPayment(pay, key, s.total);
    else addReportPayment(pay, "OTROS", s.total);
  }
  return pay;
}

function updateReportTotals(list){
  const rows = list || [];
  const total = rows.reduce((a,s)=>a+Number(s.total||0),0);
  const pay = calcReportPaymentTotals(rows);
  $("repCount").textContent = rows.length;
  $("repTotal").textContent = money(total);
  if($("repCashTotal")) $("repCashTotal").textContent = money(pay.EFECTIVO);
  if($("repCardTotal")) $("repCardTotal").textContent = money(pay.TARJETA);
  if($("repTransTotal")) $("repTransTotal").textContent = money(pay.TRANSFERENCIA);
  if($("repMixedTotal")) $("repMixedTotal").textContent = money(pay.MIXTO);
}
    ...
    <div class="sale-detail-grid">
      ...
      <div class="sale-detail-box">
        ...
        <div class="sale-detail-list">
          ...
          <div class="sale-detail-row"><span>Persona</span><strong>${escapeHtml(record.cashierName || 'SIN REGISTRO')}</strong></div>
...
async function renderReport(mode){
  ...
  for(const s of list.sort((a,b)=>b.ts-a.ts)){
    ...
    tr.innerHTML = `
      ...
      <td>${escapeHtml(saleDetailPayment(s))}</td>
      <td>${money(s.total)}</td>
      <td>
